...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mel_segment_len is not None:
            ...
            pre_out = pre_out[:, mel_start:mel_end, :]
            mel_post = mel_post[:, :, mel_start:mel_end]

        return {'mel': mel.squeeze(0), 'mel_post': mel_post.squeeze(0), 'pre_out': pre_out.squeeze(0)}
...
if __name__ == '__main__':
    ...
    files = list(Path('/Users/cschaefe/datasets/finetuning/bild_welt_masked_pre_mels').glob('**/*.pt'))
    ...
    model.eval()
    model.postnet.train()
    optimizer = optim.Adam(list(model.lstm.parameters()) + list(model.lin.parameters()) +
                                list(model.postnet.parameters()) + list(model.post_proj.parameters()), lr=1e-5)
                ...
                for i, batch in tqdm.tqdm(enumerate(val_dataloader), total=len(val_dataloader)):
                    with torch.no_grad():
                        batch = {'mel': batch['mel'].to(device), 'mel_post': batch['mel_post'].to(device),
                                 'pre_out': batch['pre_out'].to(device)}

                        ada = model.lstm(batch['pre_out'])[0]
                        ada = model.lin(ada).transpose(1, 2)
                        ada = model.postnet(ada)

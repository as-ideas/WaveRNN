...
class SeriesPredictor(nn.Module):
    ...
    def __init__(self,
        ...
        self.convs = torch.nn.ModuleList([
            BatchNormConv(in_dim, conv_dims, 3, relu=True),
            BatchNormConv(conv_dims, conv_dims, 3, relu=True),
            BatchNormConv(conv_dims, conv_dims, 3, relu=True),
        ])
        self.lin = torch.nn.Linear(conv_dims, out_dim)
...
class ConditionalSeriesPredictor(nn.Module):
    ...
    def __init__(self,
        ...
        self.convs = torch.nn.ModuleList([
            BatchNormConv(in_dim + cond_emb_dims, conv_dims, 3, relu=True),
            BatchNormConv(conv_dims, conv_dims, 3, relu=True),
            BatchNormConv(conv_dims, conv_dims, 3, relu=True),
        ])
        self.lin = torch.nn.Linear(conv_dims, 1)
        self.dropout = dropout
...
class MultiForwardTacotron(nn.Module):
    ...
    def __init__(self,
        ...
        self.dur_pred = ConditionalSeriesPredictor(in_dim=2*prenet_dims + speaker_emb_dims,
                                                   conv_dims=durpred_conv_dims,
                                                   cond_emb_dims=pitch_cond_emb_dims,
                                                   dropout=durpred_dropout)
        self.pitch_cond_pred = SeriesPredictor(in_dim=2*prenet_dims + speaker_emb_dims,
                                               conv_dims=pitch_cond_conv_dims,
                                               dropout=pitch_cond_dropout,
                                               out_dim=pitch_cond_categorical_dims)
        self.pitch_pred = ConditionalSeriesPredictor(in_dim=2*prenet_dims + speaker_emb_dims,
                                                     conv_dims=pitch_conv_dims,
                                                     cond_emb_dims=pitch_cond_emb_dims,
                                                     dropout=pitch_dropout, )
        self.energy_pred = SeriesPredictor(in_dim=2*prenet_dims + speaker_emb_dims,
    ...
    def generate(self,
        ...
        with torch.no_grad():
            ...
            dur_hat = self.dur_pred(x, pitch_cond_hat, alpha=alpha).squeeze(-1)
            if torch.sum(dur_hat.long()) <= 0:
                torch.fill_(dur_hat, value=2.)
            pitch_hat = self.pitch_pred(x, pitch_cond_hat).transpose(1, 2)

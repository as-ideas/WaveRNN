...
class LSA(nn.Module):
    def __init__(self, attn_dim, kernel_size=31, filters=32):
        super().__init__()
        self.conv = nn.Conv1d(3, filters, padding=(kernel_size - 1) // 2, kernel_size=kernel_size, bias=False)
    ...
    def forward(self, encoder_seq_proj, query, t, att_t):
        ...
        location = torch.cat([self.cumulative.unsqueeze(1), self.attention.unsqueeze(1), att_t.unsqueeze(1)], dim=1)

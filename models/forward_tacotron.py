...
class SeriesPredictor(nn.Module):
    ...
    def __init__(self, num_chars, emb_dim=64, conv_dims=256,
                 rnn_dims=64, dropout=0.5, semb_dims=256, nout=1):
        super().__init__()
...
class SeriesPredictorPitch(nn.Module):
    ...
    def __init__(self, num_chars, emb_dim=64, emb_dim_p=8, conv_dims=256,
                 rnn_dims=64, dropout=0.5, semb_dims=256):
        super().__init__()
        self.embedding = Embedding(num_chars, emb_dim)
        self.embedding_p = Embedding(4, emb_dim_p)
        self.convs = torch.nn.ModuleList([
            BatchNormConv(emb_dim + emb_dim_p + semb_dims, conv_dims, 5, relu=True),
            BatchNormConv(conv_dims, conv_dims, 5, relu=True),
            BatchNormConv(conv_dims, conv_dims, 5, relu=True),
        ])
        self.rnn = nn.GRU(conv_dims, rnn_dims, batch_first=True, bidirectional=True)
...
class ForwardTacotron(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm = nn.LSTM(2 * prenet_dims + semb_dims + 8,
                            rnn_dims,
                            batch_first=True,
                            bidirectional=True)
        ...
        self.pitch_proj = nn.Conv1d(1, 2 * prenet_dims + semb_dims + 8, kernel_size=3, padding=1)
        self.energy_proj = nn.Conv1d(1, 2 * prenet_dims + semb_dims + 8, kernel_size=3, padding=1)
        self.embedding_p = Embedding(4, 8)
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        speaker_emb = semb[:, None, :]
        speaker_emb = speaker_emb.repeat(1, x.shape[1], 1)
        x_p = self.embedding_p(pitch_p)
        x = torch.cat([x, speaker_emb, x_p], dim=2)

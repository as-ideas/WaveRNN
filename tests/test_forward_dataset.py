...
class TestForwardDataset(unittest.TestCase):
    ...
    def test_get_items(self) -> None:
        ...
        for i in range(2):
            np.save(str(mel_dir / f'{i}.npy'), mels[i], allow_pickle=False)
            np.save(str(alg_dir / f'{i}.npy'), durs[i], allow_pickle=False)
            np.save(str(pitch_dir / f'{i}.npy'), pitches[i], allow_pickle=False)
            np.save(str(energy_dir / f'{i}.npy'), energies[i], allow_pickle=False)
